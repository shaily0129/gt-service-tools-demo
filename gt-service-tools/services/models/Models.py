...
class MissionFinalAssets(BaseModel):
    patient_name: Optional[str]
    datetime_seconds: Optional[int] = None
    algo_name: str = "pyreason_basic"
    asset_final: str = ""
    asset_details: Optional[dict] = None
    confidence: float = 1.0
    ...
    def set_datetime_seconds(cls, values):
        if values.get("datetime_seconds") is None:
            values["datetime_seconds"] = int(datetime.now().timestamp())
        return values
