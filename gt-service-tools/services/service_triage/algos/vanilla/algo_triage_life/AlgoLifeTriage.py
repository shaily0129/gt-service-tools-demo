...
class LifeTriage(Triage):
    ...
    def rts_score(self, vitals_dict):
        ...
        if (
            "gcs" not in vitals_dict.keys()
            or "sbp" not in vitals_dict.keys()
            or "rr" not in vitals_dict.keys()
        ):
            return None
    ...
    def triage(self, patients: list[Patient]) -> list[TriageScore]:
        ...
        for patient in patients:
            ...
            patient_insults_keys = [
                key for key in keys_all_insults if key in record.keys()
            ]
            patient_vitals_keys = [
                key for key in keys_all_vitals if key in record.keys()
            ]
            ...
            for vital_name, threshold in self.thresholds.items():
                if vital_name in record:
                    ...
                    if value > threshold.max_value:
                        return ValueError(
                            f"{vital_name} cannot be greater than {threshold.max_value}"
                        )
                    elif value < threshold.min_value:
                        return ValueError(
                            f"{vital_name} cannot be less than {threshold.min_value}"
                        )
                    else:
