...
async def rate_response(
    ...
    try:
        ...
        for patient in body.patients:
            ...
            if not keys:
                raise HTTPException(status_code=404, detail=f"Patient ID {patient.patient_id} not found")
            
            key = keys[-1]  # Get the most recent key if there are multiple
            cached_bir_json = caching_manager.get_json(key)
            if not cached_bir_json:
                raise HTTPException(status_code=404, detail=f"Patient data for ID {patient.patient_id} not found")
            
            # Create TriageInteractionRequest from cached data
            triage_interaction_request = TriageInteractionRequest(**cached_bir_json)
