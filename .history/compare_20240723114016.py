        ...
        for patient in body.patients:
            ...
            if cached_bir_json is None:
                ...
                if cached_bir.complete:
                    patient_dict[patient.patient_id] = cached_bir
                    continue
            ...
            # Save the result and add to patient_dict
            caching_manager.save_json(key, bir.json())
            patient_dict[patient.patient_id] = bir

        # Convert patient_dict to a list and sort results by score
        sorted_results = sorted(patient_dict.values(), key=lambda x: x.triage_score.score, reverse=True)
