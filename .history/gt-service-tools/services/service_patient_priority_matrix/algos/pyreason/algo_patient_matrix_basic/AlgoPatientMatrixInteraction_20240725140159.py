...
class PatientMatrixInteraction(PatientPriority):
    ...
    def return_patient_priority_matrix(self, patients: list[Patient]) -> list[PatientMatrix]:
        ...
        patient_detail = {}
        fields = ["litter", "ambulatory", "medevac_needed", "evac_needed", "resupply_needed"]
        for field in fields:
            ...
            for t, df in enumerate(df_outer):
                if not df[field].empty:
                    for i in range(len(df["component"])):
                        patient_detail[df["component"][i]] = patient_detail.get(df["component"][i], {})
                        patient_detail[df["component"][i]][field] = df[field][i] == [1, 1]
        ...
        for field in edge_fields:
            ...
            for t, df in enumerate(df_outer):
                if not df[field].empty:
                    for i in range(len(df["component"])):
                        if df[field][i] == [1, 1]:
                            patient_detail[df["component"][i][0]] = patient_detail.get(df["component"][i][0], {})
    ...
    def run_patient_matrix_algo(self, interaction_request: PatientMatrixInteractionRequest) -> PatientMatrixInteractionRequest:
        ...
        try:
            patient = Patient(name=params.get("name", "Unknown"), category=params.get("category"))
