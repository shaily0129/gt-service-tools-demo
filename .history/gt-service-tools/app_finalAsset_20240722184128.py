...
from services.service_mission_final_assets.algos.pyreason.algo_mission_final_assets_interaction import (
...
async def mission_final_assets(
    ...
    try:
        ...
        if cached_mission_json is None:
            caching_manager.save_json(key, json.dumps(body.params))
        else:
            cached_mission = json.loads(cached_mission_json)
            if cached_mission is not None:
                return [MissionFinalAssets(**item) for item in cached_mission]
        ...
        caching_manager.save_json(key, json.dumps([asset.dict() for asset in final_assets]))
        return final_assets
